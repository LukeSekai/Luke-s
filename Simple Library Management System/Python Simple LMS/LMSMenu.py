...
class MenuWindow:
    def __init__(self, master):
        ...
        self.load_user_data_from_json()  # Load user data
        self.create_gui()
    ...
    def load_data_from_json(self):
        ...
        with open('Python Simple LMS\\library_data.json', 'r') as file:
            self.data = json.load(file)

    def load_user_data_from_json(self):  # Function to load user data
        # Load user data from JSON file
        with open('Python Simple LMS\\user_data.json', 'r') as file:
    ...
    def create_gui(self):
        ...
        interface.iconbitmap('Python Simple LMS\\icon.ico')  # Setting window icon

        # Read and resize the image using OpenCV
        image = cv2.imread('Python Simple LMS\\Library Manager\\library.png')
        ...
        centerFrame = Frame(interface, width=250, height=680, relief=SUNKEN, bg="#FEF8DD", borderwidth=5)
        centerFrame.pack(side=TOP)

        # Left frame within center frame
        centerLeftFrame = Frame(centerFrame, width=900, height=560, bg='#FEF8DD', relief=SUNKEN)
        centerLeftFrame.pack(side=LEFT)
        centerLeftFrame.pack_propagate(0)  # Prevent resizing

        # Right frame within center frame
        centerRightFrame = Frame(centerFrame, width=10, height=400, bg='#ffd2b0', relief=SUNKEN, borderwidth=1)
        centerRightFrame.pack(side=RIGHT, anchor=SW, pady=9, padx=10)
        ...
        userImage = PhotoImage(file='Python Simple LMS\\Library Manager\\user.png')
        ...
        centerRightFrameLabel = Label(centerRightFrame, text="Status: ACTIVE\n", font=('Trebuchet MS', 15 , 'bold'), padx=10, pady=2, bg="#ffd2b0", border=0)
        centerRightFrameLabel.pack()
        ...
        # Label for displaying total number of available books
        reminderLabel = Label (reminderFrame, text=" Reminder/Notes for Today ", bg='#EEDD82', fg='black', width=23, height=1, font=('Trebuchet MS', 13, 'bold'))
        ...
        # Frame for displaying total number of registered users
        frameTotalUsers = Frame(centerFrame, bg='#EEDD82')
        frameTotalUsers.place(x=908, y=20)

        # Label for displaying total number of registered users
        totalUsers = len(self.user_data)
        totalUsersBox = Label(frameTotalUsers, text=f" Total Registered\nUsers: {totalUsers}", bg='#f9ba8e', width=19, height=3, font=('Trebuchet MS', 17, 'bold'))
        totalUsersBox.pack(side=RIGHT, anchor=NE, padx=10, pady=9)
