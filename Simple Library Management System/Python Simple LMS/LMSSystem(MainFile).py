...
class Library:
    def __init__(self):
        try:
            with open('Python Simple LMS\\library_data.json', 'r') as file:  # Try to open 'library_data.json' file in read mode
    ...
    def save_database(self):
        with open('Python Simple LMS\\library_data.json', 'w') as file:  # Open 'library_data.json' file in write mode
            json.dump(self.books, file, indent=4)  # Write data from self.books into file

    # Method to add a new book to the library
    def add_book(self, title, author, book_id):
        if book_id not in self.books:  # Check if book_id is not already in books
            self.books[book_id] = {'title': title, 'author': author, 'borrower_name': 'N/A', 'available': True}  # Add book details to self.books
            ...
            messagebox.showerror("Error", "Book ID already exists in the library.")  # Show error message if book ID already exists

    # Method to checkout a book from the library
    def checkout_book(self, book_id, borrower_name):
        if book_id in self.books:  # Check if book_id is in books
            if self.books[book_id]['available']:  # Check if book is available
                self.books[book_id]['available'] = False  # Mark book as unavailable
                due_date = datetime.datetime.now() + datetime.timedelta(days=14)  # Calculate due date (14 days from now)
                self.books[book_id]['due_date'] = due_date.strftime('%Y-%m-%d')  # Store due date as string
                self.books[book_id]['borrower_name'] = borrower_name  # Store borrower's name
                self.save_database()  # Save changes to database
                messagebox.showinfo("Success", f"Book '{self.books[book_id]['title']}' checked out successfully! Please return by {due_date.strftime('%Y-%m-%d')}.")  # Show success message
            else:
                messagebox.showerror("Error", "Sorry, this book is currently checked out.")  # Show error message if book is already checked out
        else:
            messagebox.showerror("Error", "Book ID not found in the library.")  # Show error message if book ID not found

    # Method to return a book to the library
    def return_book(self, book_id):
        if book_id in self.books:  # Check if book_id is in books
            if not self.books[book_id]['available']:  # Check if book is not available (i.e., checked out)
                self.books[book_id]['available'] = True  # Mark book as available
                self.books[book_id].pop('due_date', None)  # Remove due date
                self.books[book_id]['borrower_name'] = 'N/A'  # Reset borrower's name
                self.save_database()  # Save changes to database
                messagebox.showinfo("Success", f"Book '{self.books[book_id]['title']}' returned successfully!")  # Show success message

                # Check if book is overdue and display overdue fine if applicable
                if self.is_overdue(book_id):
                    messagebox.showinfo("Overdue Fine", f"You have an overdue fine of ${self.calculate_fine(book_id)} for book '{self.books[book_id]['title']}'.")
            else:
    ...
    def is_overdue(self, book_id):
        if book_id in self.books and 'due_date' in self.books[book_id]:  # Check if book_id is in books and if 'due_date' is present
            due_date = datetime.datetime.strptime(self.books[book_id]['due_date'], '%Y-%m-%d')  # Convert due date string to datetime object
            return datetime.datetime.now() > due_date  # Return True if current date is greater than due date, else False
        return False  # Return False if book_id not in books or 'due_date' not present

    # Method to calculate overdue fine for a book
    def calculate_fine(self, book_id):
        if self.is_overdue(book_id):  # Check if book is overdue
            days_overdue = (datetime.datetime.now() - datetime.datetime.strptime(self.books[book_id]['due_date'], '%Y-%m-%d')).days  # Calculate number of days overdue
    ...
    def remove_book(self, book_id):
        if book_id in self.books:  # Check if book_id is in books
            del self.books[book_id]  # Delete book entry from self.books
            self.save_database()  # Save changes to database
            messagebox.showinfo("Success", f"Book '{self.books[book_id]['title']}' removed successfully!")  # Show success message
        else:
            messagebox.showerror("Error", "Book ID not found in the library.")  # Show error message if book ID not found
    ...
    def check_overdue_books(self):
        ...
        for book_id in self.books:
            if not self.books[book_id]['available'] and datetime.datetime.strptime(self.books[book_id]['due_date'], '%Y-%m-%d') < today:  # Check if book is checked out and due date has passed
                overdue_books.append(self.books[book_id]['title'])  # Add title of overdue book to list
        if overdue_books:  # If there are overdue books
            ...
            for book_id in overdue_books:
                fine = self.calculate_fine(book_id)  # Calculate fine for overdue book
                borrower_name = self.books[book_id]['borrower_name']  # Get borrower's name for overdue book
                messagebox.showinfo(f"Overdue Fine for {borrower_name}", f"You have an overdue fine of ${fine} for book '{self.books[book_id]['title']}'.")  # Show overdue fine message
        else:
...
def checkout_book_callback():
    ...
    if book_id and borrower_name:
        library.checkout_book(book_id, borrower_name)  # Pass borrower name
    else:
        messagebox.showerror("Error", "Please enter the book ID and borrower name.")
...
def remove_book_callback():
    book_id = entryID.get()
    if book_id:
        # Display success message before removing the book
        messagebox.showinfo("Success", f"Book '{library.books[book_id]['title']}' removed successfully!")
        library.remove_book(book_id)
    else:
        messagebox.showerror("Error", "Please enter the book ID to remove.")
...
def populate_listbox_sort_by_id():
    ...
    sorted_books = sorted(library.books.items(), key=lambda x: x[0])  # Sort by book_id (primary key)
    
    for book_id, details in sorted_books:
        # Use get method to handle cases where 'borrower_name' or 'due_date' may not be present
        borrower_name = details.get('borrower_name', 'N/A')
        due_date = details.get('due_date', 'N/A')
        # Format book details into a table-like manner
        book_info = f"Title: {details['title'].ljust(30)} | Author: {details['author'].ljust(24)} | Book ID: {book_id.ljust(15)} | Borrower Name: {borrower_name.ljust(20)} | Status: {'Available' if details['available'] else 'Checked Out'.ljust(10)} | Due Date: {due_date}"
...
addBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\addbook.png')
removeBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\removeBook.png')
checkoutBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\checkout.png')
displayBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\displaybook.png')
overdueBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\overdue.png')
returnBook_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\returnbook.png')
menu_cv_image = cv2.imread('Python Simple LMS\\Library Manager\\menu.png')
...
icon_image = PhotoImage(file='Python Simple LMS\\icon.png')
...
# Buttons
AddBookbutton = Button(frameButton1, text="  Add Book", font=('Trebuchet MS', 20 , 'bold'), fg='black', command=add_book_callback, bg="#aab396", height=49, width=320, image=addBook_photo_image, compound=LEFT, relief=RAISED, anchor='w')
...
CheckoutBookbutton = Button(frameButton3, text="  Check Out", font=('Trebuchet MS', 20 , 'bold'), fg='black', command=checkout_book_callback, bg="#aab396", height=49, width=320, image=checkoutBook_photo_image, compound=LEFT, relief=RAISED, anchor='w')
...
CheckOverdueBooksbutton = Button(frameButton5, text="  Overdue Books", font=('Trebuchet MS', 20 , 'bold'), fg='black', command=check_overdue_books_callback, bg="#aab396", height=49, width=320, image=overdueBook_photo_image, compound=LEFT, relief=RAISED, anchor='w')
...
MenuButton = Button(frameButton7, text="  Menu", font=('Trebuchet MS', 20 , 'bold'), fg='black', command=lambda: openLMSMenu(m_window), bg="#aab396", height=55, width=320, image=menu_photo_image, compound=LEFT, relief=RAISED, anchor='w')
